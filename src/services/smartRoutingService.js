...
class SmartRoutingService {
  async analyzeProblem(problemText, language = "uz", orgId = null) {
    try {
      ...
      return {
        organization:
          ...
          response.text?.match(/Idora[:\s]+(.+)/)?.[1] ||
          "Davlat xizmatlari markazi",
        department:
          response.department ||
          response.text?.match(/Bo'lim[:\s]+(.+)/)?.[1] ||
          "Maslahat bo'limi",
...
Use real organization and department names from the Republic of Uzbekistan.`,
  ...
  async analyzeWithAI(problemText, language) {
    ...
    return await askAI(problemText, language, {
      context: context[language] || context.uz,
    });
  }
